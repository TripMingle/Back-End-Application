...
- [3. 개발 과정](#3-개발-과정)
  - [3-1. 게일 섀플리 알고리즘을 사용한 유저 매칭 서비스](#3-1-게일-섀플리-알고리즘을-사용한-유저-매칭-서비스)
- [4. 프로젝트 관리](#4-프로젝트-관리)
...
### 3-1. 게일 섀플리 알고리즘을 사용한 유저 매칭 서비스
  기존의 여행 성향이 유사한 유저 추천 방식에 더해서, 지금 당장 여행 일정이 맞는 동행을 구하고자 하는 유저끼리 매칭해주는 서비스를 도입하였습니다.
  <br />
  이 매칭 서비스는 유저에게 일정이 맞는 동행 구인 게시물을 매칭해줍니다.
  
  <img src="./image/gale1.png" width="800">
  기존 게일-섀플리 알고리즘은 위의 결과처럼 구직자와 회사의 N:N 집단에서 1:1 매칭을 제공합니다.
<br /><br /><br />
  <img src="./image/gale2.png" width="800">
    이 서비스에서는 이를 변형하여 동행신청유저와 동행모집유저의 N:M 집단에서 다대다 매칭을 제공하는 방식으로 리팩토링 하였습니다.
<br /><br />

### 3-2. 웹소켓 기반 동행 채팅 서비스
